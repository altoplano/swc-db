...
*/
  ...
  const bool is_matching_begin(const DB::Cell::Key& key) const {
    return range_begin.empty() || 
           range_begin.compare(key, range_begin.size(), true) != Condition::LT;
  }

  const bool is_matching_end(const DB::Cell::Key& key) const {
    return range_end.empty() || 
           range_end.compare(key, range_end.size(), true) != Condition::GT;
  }
  ...
  void apply_possible_range_begin(DB::Cell::Key& begin) const {
    if(!offset_key.empty()) {
      ...
      if(!++ok)
        begin.free();
      else if(ok != key_start.size() && ok != begin.size())
      ...
      if(!++ok)
        begin.free();
      else if(ok != key_finish.size() && ok != begin.size())
        begin.remove(ok, true);
    }
  }
  ...
  void apply_possible_range_end(DB::Cell::Key& end) const {
    if(!range_end.empty()) {
      ...
      else if(++ok < key_start.size() && ok != end.size())
        end.remove(ok, true);
    }
    ...
    /* NOT POSSIBLE
      ...
      else if(++ok < key_finish.size() && ok != end.size())
        end.remove(ok, true);
